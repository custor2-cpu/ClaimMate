...
export function buildFallbackReport(ml: MLAnalysisResult): AgentReport {
  ...
  const notice_letter_template = `내용증명
...
제목: ${ml.category} 관련 ${ml.dispute_type}에 따른 계약 해지 및 환급 청구

1. 귀 사의 무궁한 발전을 기원합니다.

2. 발신인(이하 '소비자')은 ${contractDate}경 귀 사와 아래와 같이 계약을 체결하였습니다.
   - 상담/계약 내용: ${ml.input.text}
   - 결제 금액: ${amountText}

3. 그러나 다음과 같은 사유로 계약 해지 및 정당한 환급을 요청드립니다.
   - 분쟁 유형: ${ml.dispute_type}

4. 이는 공정거래위원회 고시 「소비자분쟁해결기준」 및 관련 법령에 근거한 정당한 권리 행사입니다.
   (근거: ${knowledge.legal_basis})

5. 이에 따른 예상 환급 범위는 ${estimated_refund} 수준으로 판단되며, 아래 계좌로 환급하여
   주시기 바랍니다.
   - 환급 계좌: [은행명] [계좌번호] (예금주: [소비자 성명])

6. 본 통지 수령일로부터 7일 이내에 성실한 답변과 환급 조치를 이행하여 주시기 바라며, 미이행
   시 한국소비자원 피해구제 신청, 관할 법원 민사조정/소송 등 법적 절차를 진행할 수 있음을
   알려드립니다.

첨부: ${knowledge.proof_documents.join(", ")}
