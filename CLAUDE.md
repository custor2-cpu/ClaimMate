...
- `predictor.py`는 매 요청(subprocess)마다 재학습하면 느리므로(~7초) 학습된 모델을
  ...
  캐시에 담기는 객체 구성 자체가 바뀌는 코드 변경(예: 유사도 계산 로직 교체)을 할 때는
  `_CACHE_SCHEMA_VERSION`도 함께 올려야 한다 — 데이터가 그대로면 mtime/건수만으로는
  캐시 키가 안 바뀌어 예전 스키마의 캐시를 그대로 읽어 오류가 나거나 조용히 옛 로직으로
  동작할 수 있다.
- **유사 사례 검색: 코사인 유사도 → BM25 교체**: 원래 `_vectorizer`(char n-gram TF-IDF)로
  만든 벡터의 코사인 유사도로 Top-3 유사 사례를 뽑았는데, 실사용 중 "이사한 곳이 서비스
  불가 지역이라 인터넷을 해지했더니 위약금을 청구당했다"는 상담에 대해, 사안이 정확히
  동일한 실제 공공데이터 사례(문장이 길고 "직장 이전", "오피스텔이라 주소이전 불가" 같은
  부가 정보가 많음)가 사안은 다르지만 문장이 짧고 구어체인 `SYNTHETIC_CASES` 예시보다
  낮은 유사도를 받는 역전 현상이 확인됐다. char n-gram TF-IDF 코사인 유사도는 문서가
  길수록(공공데이터 특유의 격식체+부가 설명) 벡터가 희석되어, 순수 문자열 겹침 비율만
  보면 짧고 구어체인 문서가 부당하게 유리해진다. `predict()`의 유사 사례 랭킹만
  BM25(`_bm25_similarity()`, 문서 길이를 평균 길이 대비 명시적으로 정규화)로 교체해
  해결했다 — 분류기(`_classifier`)는 여전히 기존 `_tfidf_matrix`/코사인 기반 K-Means를
  그대로 쓰므로 분류 정확도에는 영향이 없다. BM25용 원시 빈도 행렬(`_count_matrix`)은
  `_vectorizer`와 동일한 vocabulary를 공유하는 별도 `CountVectorizer`로 만들며, 유사도는
  쿼리 자신을 "평균 길이 문서"로 가정했을 때의 점수를 100으로 놓고 정규화한다.
...
- **`estimated_refund` 계산 시 이미 사용한 만큼 차감 누락 수정**: 회원권/이용권처럼 정해진
  횟수·기간 중 일부를 이미 사용한 계속거래 계약(헬스장·피부관리실 등)에서, LLM이
  `estimated_refund`를 "결제 총액 - 위약금(10%)"으로만 계산해 이미 사용한 부분까지
  환급 대상에 포함시키는 오류가 실사용 중 발견됐다(예: "10회권 800,000원 중 3회 사용"
  사례에서 720,000원으로 계산 — 실제로는 미사용분 7/10인 560,000원에서 위약금을
  공제해야 함). 시스템 프롬프트 지시만으로는 고쳐지지 않았고, **JSON 스키마의
  `estimated_refund` 필드 `description`에 같은 규칙을 한 번 더 명시**하고 나서야 반영됐다
  — 구조화 출력(Structured Output)에서는 필드 레벨 설명이 시스템 프롬프트 본문의 규칙보다
  더 강하게 반영되는 경향이 있다는 뜻으로, 비슷한 계산 오류를 프롬프트만으로 못 고칠 때는
  스키마 필드 설명도 함께 수정해볼 것. (수정 확인 중 `.next/` 캐시가 남아 있어 코드를
  고쳐도 즉시 반영되지 않는 경우가 있었다 — 재현이 안 될 때는 `.next/`를 지우고 dev
  서버를 재시작해 확인할 것.)
